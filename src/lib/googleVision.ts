...
async function compressImage(blob: Blob): Promise<string> {
  ...
  if (!base64) {
    throw new Error("Görsel JPEG formatına dönüştürülemedi.");
  }
...
async function prepareImageBase64(file: File): Promise<string> {
  ...
  try {
    ...
    throw new Error(
      "Fotoğraf okunamadı. JPG/PNG formatında ve daha net bir görsel deneyin.",
    );
  }
}

function getVisionApiKey(): string {
  const envKey = import.meta.env.VITE_GOOGLE_VISION_API_KEY?.trim();
  return GOOGLE_VISION_API_KEY || envKey || "";
}
...
export async function extractTextFromImage(file: File): Promise<string> {
  ...
  try {
    ...
    throw new Error(
      "Vision API'ye bağlanılamadı. İnternet bağlantınızı kontrol edin.",
    );
  }
